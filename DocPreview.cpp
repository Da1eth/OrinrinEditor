...
#ifndef _ATL
#error IE 컴포넌트를 사용하려면 ATL이 필요합니다.
#endif
...
//#if _ATL_VER == 0x0800
#if _ATL_VER == 0x0E00
#define ATL_AX_WIN	TEXT("AtlAxWin140")
#else
#error 맞는 ATL 버전을 사용해야 합니다.
#endif
...
static CONST CHAR	gcacWeek[7][4] = { ("Sun"), ("Mon"), ("Tue"), ("Wed"), ("Sat"), ("Fri"), ("Thu") };

static  CHAR	gacResHeaderFmt[] = { "<dt>%d NamelessTuna (verygood)<br>%d-%02d-%02d(%s) %02d:%02d:%02d</dt> <dd>" };	//	
...
VOID	Pvw_OnKey(HWND, UINT, BOOL, INT, UINT);			//!<	2024kai Escでプレビューを閉じる
//-------------------------------------------------------------------------------------------------
...
#pragma message ("全プレ書換時に、スクロールバーの位置覚えておいて、そこまでScrollさせる？")
	...
	if( SUCCEEDED( AtlAxGetControl( ghIEwnd, &comPunkIE ) ) )
	{
		...
		{
			NotifyBalloonExist( TEXT("IE 컴포넌트 초기화에 실패했습니다."), TEXT("꺄아아아아아악!"), NIIF_ERROR );
...
LRESULT CALLBACK PreviewWndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	switch( message )
	{
		...
		HANDLE_MSG(hWnd, WM_KEYDOWN, Pvw_OnKey); //2024kai
...
HRESULT PreviewPageWrite( INT iViewPage )
{
	...
	if( !(gpcHtmlHdr) )
	{
		NotifyBalloonExist( TEXT("미리보기용 템플릿 파일이 없습니다."), TEXT("꺄아아아아아악!"), NIIF_ERROR );
		return E_HANDLE;
	}
