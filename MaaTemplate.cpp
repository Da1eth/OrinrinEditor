...
BOOLEAN Maa_OnCreate( HWND hWnd, LPCREATESTRUCT lpCreateStruct )
{
	...
	//	先頭から・表示位置からラヂオボタン
	CreateWindowEx( 0, WC_BUTTON, TEXT("현재 페이지부터"), WS_VISIBLE | WS_CHILD | WS_GROUP | BS_AUTORADIOBUTTON | BS_VCENTER, 183, iTfTop, 80, 23, hWnd, (HMENU)IDB_MAA_TXTFIND_TOP_GO, lcInst, NULL );
...
//		stOpenFile.nFilterIndex    = 1;
	...
	//	stOpenFile.lpstrInitialDir = 
		stOpenFile.lpstrTitle      = TEXT("기존 파일이나 새로 작성할 파일의 이름을 입력해주세요.");
		...
		case WM_COMMAND:
			...
			switch( id )
			{
				...
				case IDOK:
					...
					if( 1 >= count )
					{
						MessageBox( hDlg, TEXT("리스트업이 되어 있지 않기 때문에\r\n트리를 만들 수 없습니다."), TEXT("오린의 안내"), MB_OK | MB_ICONERROR );
						return (INT_PTR)TRUE;
					}
...
HRESULT TreeLoadDirCheck( HWND hDlg, HWND hTvWnd )
{
	...
	{
		ShowWindow( hWorkWnd, SW_HIDE );
		MessageBox( hDlg, TEXT("파일이 선택되어 있지 않기 때문에\r\n트리를 만들 수 없습니다."), TEXT("오린의 안내"), MB_OK | MB_ICONERROR );
		return E_ABORT;
	}
...
//	HWND	hWndChild;
	...
	switch( message )
	{
		...
		case WM_INITDIALOG:
			...
			stLvColm.pszText = TEXT("파일 이름");	stLvColm.cx = 250;	stLvColm.iSubItem = 0;	ListView_InsertColumn( hWorkWnd, 0, &stLvColm );
...
HRESULT OpenProfileInitialise( HWND hWnd )
{
	...
	if( hWnd )	//	ロード
	{
		...
		AppendMenu( ghProfHisMenu, MF_STRING, IDM_OPEN_HIS_CLEAR, TEXT("기록 삭제") );
		...
		{
			//	オーポン履歴が無い場合
			InsertMenu( ghProfHisMenu, 0, MF_STRING | MF_BYPOSITION | MF_GRAYED, IDM_OPEN_HIS_FIRST, TEXT("(비었음)") );
		}
		else
		{
			//	オーポン履歴を並べる
