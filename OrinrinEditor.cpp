...
TEXT("이 프로그램은 개인에 의해 번역되었으며, 원본을 훼손할 의도가 없습니다. \r\n\r\n")
};
//-------------------------------------------------------------------------------------------------
...
//	DWORD	dwStyle;
	...
	InsertMenu( hSubMenu, 0, MF_BYPOSITION, IDM_POSITION_RESET, TEXT("창 배치 리셋") );
...
HRESULT OpenHistoryInitialise( HWND hWnd )
{
	...
	if( hWnd )
	{
		...
		AppendMenu( ghHistyMenu, MF_STRING, IDM_OPEN_HIS_CLEAR, TEXT("열어본 파일 기록 지우기") );
		//文字列固定はあまりイクナイ
...
#pragma message ("ファイルオーポン履歴の基点メニュー、位置に注意")
		...
		ModifyMenu( hSubMenu, 2, MF_BYPOSITION | MF_POPUP, (UINT_PTR)ghHistyMenu, TEXT("열어본 파일 기록(&H)") );
		//文字列固定はあまりイクナイ
...
BOOLEAN SelectDirectoryDlg( HWND hWnd, LPTSTR ptSelDir, UINT_PTR cchLen )
{
	...
	stBrowseInfo.lpszTitle		 = TEXT("AA가 있는 폴더를 선택해주세요.");	//	ツリービューの上部に表示される文字列
...
UINT DocHugeFileTreatment( UINT dStyle )
{
	...
	if( 1 == dStyle )
	{
		iRslt = MessageBox( bVisible ? ghMainWnd : NULL, TEXT("대형 파일을 열고 있습니다. 빠른 로딩을 위해 파일을 분할할까요?\r\n\r\n　　네 : 분할해서 열기\t\n　　아니오 : 분할하지 않기\r\n　　취소：파일 열지 않기"), TEXT("매우 큰 파일 안내"), MB_YESNOCANCEL | MB_ICONQUESTION );
	...
	{
		MessageBox( bVisible ? ghMainWnd : NULL, TEXT("파일 내용에 이상이 있어 불러오기를 중단했습니다."), TEXT("개행 없는 파일?"), MB_OK | MB_ICONERROR );
		dMode = 2;
	}
	else
	{
