...
VOID Maa_OnContextMenu( HWND hWnd, HWND hWndContext, UINT xPos, UINT yPos )
{
	...
	if( ghFavLtWnd == hWndContext )
	{
		...
		TRACE( TEXT("리스트 박스 컨텍스트 %d"), curSel );
		if( 0 > curSel )	return;

		ListBox_GetText( ghFavLtWnd, curSel, atSelName );
		StringCchPrintf( atMenuStr,  MAX_PATH, TEXT("[ %s ]에서 탭을 추가"), atSelName );
		StringCchPrintf( atMenuStr2, MAX_PATH, TEXT("[ %s ]그룹을 삭제"), atSelName );
	...
	if( ghTreeWnd == hWndContext )
	{
		...
		//	プロフ履歴入替
		ModifyMenu( hSubMenu, IDM_OPEN_HISTORY, MF_BYCOMMAND | MF_POPUP, (UINT_PTR)ghProfHisMenu, TEXT("열어본 파일 목록(&H)") );
...
#pragma message ("MAAタブの固定タブの判別に注意")
		...
		switch( dRslt )
		{
			...
			case  IDM_AATABS_ALLDELETE:	
				iRslt = MessageBox( hWnd, TEXT("서브 탭을 전부 종료합니다.\r\n이대로 진행할까요?"), TEXT("탭 닫기 안내"), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2 );
...
//	SHFILEINFO	stShFileInfo;
	...
	StatusBarMsgSet( SBMAA_FILENAME, TEXT("트리 구축 중입니다.") );
...
UINT TreeNodeExtraAdding( LPCTSTR ptPath )
{
	...
	if( 0 < id )
	{
		MessageBox( GetDesktopWindow( ), TEXT("이미 등록되어 있습니다."), TEXT("오린의 안내"), MB_OK | MB_ICONINFORMATION );
		return id;
	}
...
HRESULT TabMultipleRestore( HWND hWnd )
{
	...
	//2024kai おりんりん起動時にプロファイルにある初めの副タブを選択する
	if (iCount > 0)
	{
		TabCtrl_SetCurSel(ghTabWnd, 2);
		TabMultipleSelect(hWnd, 2, 0);
	}
