...
//2024kai ページリストのByteと行がちらつく対策のCheck用変数
static INT iPageCh = 0;
static INT dByteCh = 0;
static INT dLineCh = 0;
//-------------------------------------------------------------------------------------------------
...
HWND PageListInitialise( HINSTANCE hInstance, HWND hParentWnd, LPRECT pstFrame )
{
	...
	StringCchCopy( atBuff, MAX_STRING, TEXT("페이지 정보 갱신") );						gstPgTlBarInfo[ 8].iString = SendMessage( ghToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuff );
...
//リストビュー	LVS_SHOWSELALWAYS
	...
	stLvColm.pszText = TEXT("용량");	stLvColm.cx =  45;	stLvColm.iSubItem = 2;	ListView_InsertColumn( ghPageListWnd, 2, &stLvColm );
	stLvColm.pszText = TEXT("줄 수");	stLvColm.cx =  45;	stLvColm.iSubItem = 3;	ListView_InsertColumn( ghPageListWnd, 3, &stLvColm );
...
//	ListView_GetSelectedCount
	...
	switch( id )
	{
		...
		case IDM_PAGEL_DETAIL:
			TRACE( TEXT("페이지 정보 갱신") );
			return;

		default:	TRACE( TEXT("미실장[%d]"), id );	return;
	}
...
HRESULT PageListViewChange( INT iPage, INT iPrePage )
{
	...
	//2024kai InvalidateRectだとページリスト全体がちらつくため以下で対応（該当行だけちらつかせる）
	//この部分のちらつき改善だけで10時間かかって苦痛でしたが、出来た時は少し泣きました
	SendMessage(ghPageListWnd, LVM_REDRAWITEMS, iPrePage, iPrePage);
	SendMessage(ghPageListWnd, LVM_REDRAWITEMS, gixPreviSel, gixPreviSel);
