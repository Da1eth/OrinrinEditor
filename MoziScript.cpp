...
//	StringCchCopy( atBuffer, MAX_STRING, TEXT("アドバンズド") );					gstMztbInfo[4].iString = SendMessage( ghMoziToolBar, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	...
	CreateWindowEx( 0, WC_BUTTON, TEXT("작업 후 닫기"), WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 128, 2, 170, 23, ghMoziToolBar, (HMENU)IDCB_MZSCR_QUICKCLOSE, hInst, NULL );
	...
	//	文字間透過CBX
	CreateWindowEx( 0, WC_BUTTON, TEXT("문자 사이 공백 투과"), WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 130, gdToolBarHei, 120, MZ_PARAMHEI, ghMoziWnd, (HMENU)IDCB_MZSCR_TRANSPARENT, hInst, NULL );
...
//	gpfOrgSettiLvProc = SubclassWindow( ghSettiLvWnd, gpfSettiLvProc );
	...
	stLvColm.pszText = TEXT("파일 이름");	stLvColm.cx = 200;	stLvColm.iSubItem = 0x00;	ListView_InsertColumn( ghSettiLvWnd, 0, &stLvColm );
	stLvColm.pszText = TEXT("파일 경로");	stLvColm.cx = 500;	stLvColm.iSubItem = 0x01;	ListView_InsertColumn( ghSettiLvWnd, 1, &stLvColm );
...
//表示・位置決め半透明フローティングウインドー
	ghMoziViewWnd = CreateWindowEx( WS_EX_TOOLWINDOW | WS_EX_LAYERED, MOZIVIEW_CLASS,
		TEXT("위치"), WS_POPUP | WS_THICKFRAME | WS_CAPTION | WS_VISIBLE, 0, 0, 160, 120, NULL, NULL, hInst, NULL);
...
//	TRACE( TEXT("%d x %d"), xEt, yEt );
	...
	StringCchPrintf( atBuffer, SUB_STRING, TEXT("MOZI %d번째 줄 %d도트"), dLine, xSb );
