...
//	if( ghPtWnd == hPtWnd )	呼びだした方によって、ラスト位置リロード・底までしなくて良いか
	...
	if( gbThumb )	//	サムネモード
	{
		...
		StringCchCopy( atCaption, SUB_STRING, TEXT("MAA 섬네일") );
	}
	else
	{
	...
	if( !(ghNonItemDC) )	//	穴埋め描画用ビットマップ作成
	{
		...
		iRslt = DrawText( ghNonItemDC, TEXT("빈 보드"), 7, &rect, DT_CENTER | DT_VCENTER | DT_NOPREFIX | DT_NOCLIP | DT_SINGLELINE );
...
VOID Drt_OnContextMenu( HWND hWnd, HWND hWndContext, UINT xPos, UINT yPos )
{
	...
	if( gbThumb )	//	サムネ側なら
	{
		...
		//	文字列変更
		ModifyMenu( hSubMenu, IDM_DRAUGHT_CLOSE,     MF_BYCOMMAND | MFT_STRING, IDM_DRAUGHT_CLOSE, TEXT("섬네일 닫기(&Q)") );
...
HRESULT DraughtItemExport( HWND hWnd, LPTSTR ptPath )
{
	...
	stOpenFile.lpstrFilter     = TEXT("여러 줄 템플릿 파일(*.mlt)\0*.mlt\0全ての形式(*.*)\0*.*\0\0");
	...
	stOpenFile.lpstrTitle      = TEXT("어떤 이름으로 저장할까요?");
	...
	TRACE( TEXT("파일 저장 대화상자 통과 완료 [%X]"), wrote );
	...
	MessageBox( hWnd, TEXT("파일을 저장했습니다."), TEXT("일했꼬꼬"), MB_OK | MB_ICONINFORMATION );
