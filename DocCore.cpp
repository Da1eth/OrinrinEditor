...
LPARAM DocMultiFileClose( HWND hWnd, LPARAM uqNumber )
{
	...
	{
		StringCchPrintf( atBuffer, MAX_PATH, TEXT("\r\n[%s] 파일이 저장되지 않았습니다. \r\n지금 저장하고 종료할까요?"), PathFindFileName( gitFileIt->atFileName ) );
		iRslt = MessageBox( hWnd, atBuffer, TEXT("꺄아아아아아악"), MB_YESNOCANCEL | MB_ICONQUESTION );
...
INT DocFileCloseCheck( HWND hWnd, UINT dMode )
{
	...
	for( itFiles = gltMultiFiles.begin(); itFiles != gltMultiFiles.end(); itFiles++ )
	{
		if( itFiles->dModify )
		{
			StringCchPrintf( atMessage, BIG_STRING, TEXT("\r\n%s 파일이 저장되지 않았습니다. 지금 저장할까요?"), itFiles->atFileName[0] ? PathFindFileName( itFiles->atFileName ) : itFiles->atDummyName );
			rslt = MessageBox( hWnd, atMessage, TEXT("꺄아아아아아악"), MB_YESNOCANCEL | MB_ICONQUESTION );
	...
	//2024kai おりんりんに休暇を与える（でも保存してない時はよろしくお願いします！）
...
//	gixFocusPage = dFocusBuf;	//	終わったら戻す
	...
	StringCchPrintf( atTitle, 64, TEXT("%d 페이지를 복사했습니다."), gixDropPage + 1 );
