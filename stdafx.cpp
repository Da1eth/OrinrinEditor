...
INT_PTR MessageBoxCheckBox( HWND hWnd, HINSTANCE hInst, UINT dStyle )
{
	...
	{
		case  0:	//	統合確認
			StringCchCopy( stMsg.atMsg1, MAX_PATH, TEXT("페이지를 통합하면 되돌릴 수 없습니다.") );
			StringCchCopy( stMsg.atMsg2, MAX_PATH, TEXT("정말로 진행할까요?") );
			number = VL_PCOMBINE_NM;
			break;

		case  1:	//	分離確認
			StringCchCopy( stMsg.atMsg1, MAX_PATH, TEXT("페이지를 분할하면 되돌릴 수 없습니다.") );
			StringCchCopy( stMsg.atMsg2, MAX_PATH, TEXT("정말로 분할할까요?") );
			number = VL_PDIVIDE_NM;
			break;

		case  2:	//	削除確認
			StringCchCopy( stMsg.atMsg1, MAX_PATH, TEXT("페이지를 삭제하면 되돌릴 수 없습니다.") );
			StringCchCopy( stMsg.atMsg2, MAX_PATH, TEXT("정말로 삭제할까요?") );
...
LRESULT ExceptionMessage( LPCSTR pcExpMsg, LPCSTR pcFuncName, UINT dLine, LPARAM lReturn )
{
	...
	StringCchPrintfA( acMessage, BIG_STRING, ("오류 발생＜%s＞[%s:%u]\r\n프로그램을 종료합니다."), pcExpMsg, pcFuncName, dLine );
