...
INT DocDiffAdjExec( PINT pxDot, INT yLine )
{
	...
	if( !(bIsSpace) )
	{
		NotifyBalloonExist( TEXT("공백 문자가 존재하는 위치로 커서를 이동해주세요."), TEXT("조정 불가"), NIIF_ERROR );
		return 0;
	}
	//今現在の空白幅を確認
...
//まず全角半角で埋めて、半角が多いようならピリオド付けて再計算
	...
	{
		NotifyBalloonExist( TEXT("공백 문자의 길이가 너무 작아서 조정할 수 없습니다."), TEXT("너무 좁아요"), NIIF_ERROR );
		return 0;
	}
	...
	if( !(ptPlus) )
	{
		NotifyBalloonExist( TEXT("길이를 조정하지 못했습니다."), TEXT("자동 조정 실패"), NIIF_ERROR );
		return 0;
	}
...
HRESULT DocRightSlide( PINT pXdot, INT dLine )
{
	...
	if( 0 > dPaDot )
	{
		NotifyBalloonExist( TEXT("문자가 바깥으로 너무 튀어나와 있습니다."), TEXT("실패..."), NIIF_ERROR );
		return E_FAIL;
	}
