...
INT LayerImportDotCheck(HWND, LONG, LONG, INT); //2024kai
//-------------------------------------------------------------------------------------------------
...
//	TBADDBITMAP	stToolBmp;
	...
	StringCchCopy( atBuffer, MAX_STRING, TEXT("끼워넣기") );	gstTBInfo[0].iString = SendMessage( hToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	StringCchCopy( atBuffer, MAX_STRING, TEXT("덮어쓰기") );	gstTBInfo[1].iString = SendMessage( hToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	StringCchCopy( atBuffer, MAX_STRING, TEXT("클립보드로 복사") );		gstTBInfo[3].iString = SendMessage( hToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	StringCchCopy( atBuffer, MAX_STRING, TEXT("내용 편집") );	gstTBInfo[5].iString = SendMessage( hToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	...
	switch( id )
	{
		case IDE_LYB_TEXTEDIT:	//	レイヤ非表示にしてもKILLFOCUS出る
			...
			if( EN_KILLFOCUS == codeNotify )
			{
				...
				//2024kai テキスト編集オフでレイヤボックスのフォーカスが外れないように変更
				//ViewFocusSet(  );
			}
			break;
		...
		case IDCB_LAYER_EDGE_BLANK:	//	白ヌキするか
			if( CBN_SELCHANGE == codeNotify )
			{
				...
				SetFocus(hWnd); //2024kai コンボボックス変更後のEnterでレイヤボックス上書き貼り付けが効くように対応
			}
			break;
#endif
...
VOID Lyb_OnKey( HWND hWnd, UINT vk, BOOL fDown, int cRepeat, UINT flags )
{
	...
	if( fDown )
	{
		switch( vk )
		{
			case VK_RIGHT:	TRACE( TEXT("右") );	dotLeft = 1;	break;
			case VK_LEFT:	TRACE( TEXT("左") );	dotLeft = -1;	break;
			...
			case  VK_RETURN:	TRACE(TEXT("Enter"));	Lyb_OnCommand(hWnd, IDM_LYB_OVERRIDE, NULL, 0);	break;
			//2024kai Escでレイヤボックスを閉じる
			case  VK_ESCAPE:	TRACE(TEXT("Esc"));	DestroyWindow(hWnd);	break;
			default:	return;
		}
	}
...
//	TRACE( TEXT("%d x %d"), xEt, yEt );
	...
	StringCchPrintf( atBuffer, SUB_STRING, TEXT("레이어 %d번째 줄 %d도트"), dLine, xSb );
