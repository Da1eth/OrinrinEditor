...
//	INT			iNewPage;
	...
	// 2024kai WS_EX_COMPOSITEDでメインエディタのちらつき改善
	ghViewWnd = CreateWindowEx( WS_EX_COMPOSITED, EDIT_VIEW_CLASS, NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL,
...
HRESULT ViewNowPosStatus( VOID )
{
	...
	StringCchPrintf( atString, SUB_STRING, TEXT("커서 %d번째 줄 %d도트 %d문자"), gdDocLine + 1, gdDocXdot, gdDocMozi );
...
// 2024kai 1文字ずつhdcを準備すると重いので共通のhdcを使い回して1行ごとに解放させる
// 貼り付けに0.7秒かかるAAが0.2秒以下まで高速化！紆余曲折あって12時間かかり無理だと絶望してたので出来て泣きました
INT GetHdcC() {
	hdcCommonF = TRUE;
	hdcC = GetDC(ghViewWnd);
	hFtOldC = SelectFont(hdcC, ghAaFont);
	return 0;
}
INT ReleaseHdcC() {
...
{
	CONST  TCHAR	*catTexts[] = { { TEXT("[사각형 선택]") }, { TEXT("[브러시]") },
		{ TEXT("[다중 선택]") }, { TEXT("[공백 표시]") } };
...
//		case IDM_FIND_JUMP_PREV:	FindStringJump( 1, &gdDocXdot, &gdDocLine, &gdDocMozi );	break;
        ...
        case IDM_INSTAG_CLOSPO:
		case IDM_INSTAG_BLUE:
		case IDM_INSTAG_BLACK:
		case IDM_INSTAG_RED:
		case IDM_INSTAG_GREEN:
        case IDM_INSTAG_CLOCLR:		ViewInsertColourTag(id);	break;
