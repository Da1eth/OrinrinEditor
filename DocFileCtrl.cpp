...
//	LPARAM	dNumber;
	...
	stOpenFile.lpstrFilter     = TEXT("아스키 아트 파일 ( mlt, ast, txt )\0*.mlt;*.ast;*.txt\0全ての形式(*.*)\0*.*\0\0");
...
HRESULT DocDoOpenFile( HWND hWnd, LPTSTR ptFile )
{
	...
	if( !(dNumber) )
	{
		MessageBox( hWnd, TEXT("불러오기에 실패했습니다."), TEXT("꺄아아아아아악!"), MB_OK | MB_ICONERROR );
		return E_HANDLE;
	}
	else
	{
...
//複数ファイル、各ファイルをセーブするには？
	for( itFile = gltMultiFiles.begin(); itFile != gltMultiFiles.end(); itFile++ )
	{
		...
		{
			NotifyBalloonExist( TEXT("파일 백업에 실패했습니다."), TEXT("꺄아아아아아악!"), NIIF_ERROR );
	...
	if( gbAutoBUmsg ){	NotifyBalloonExist( TEXT("작업 중인 파일을 백업했습니다."), TEXT("백업 완료 안내"), NIIF_INFO );	}
...
//既存の拡張子がASTなら、それを優先する
	...
	if( (bStyle & D_RENAME) || bNoName )
	{
		...
		stSaveFile.lpstrFilter     = TEXT("[ANSI]아스키 아트 파일 ( mlt, ast, txt )\0*.mlt;*.ast;*.txt\0[UTF8]아스키 아트 파일 ( mlt, ast, txt )\0*.mlt;*.ast;*.txt\0\0");
...
//		stSaveFile.lpstrInitialDir = 
		...
		{	//	名無しのままエクスポートしようとしてたら無効
			...
			{
				MessageBox( hWnd, TEXT("내보내기를 하려면 먼저 파일을 저장해야 합니다."), TEXT("저장을 해주세요"), MB_OK | MB_ICONINFORMATION );
				return E_FAIL;
			}
		}
	...
	else	//	既存の拡張子があったら
	{
		...
		if( !( StrCmp( atExBuf, aatExte[1] ) ) )
		{
			...
			{
				mbRslt = MessageBox( hWnd, TEXT("MLT 확장자로 저장하면 페이지 이름이 사라집니다.\r\n이대로 저장할까요?"), TEXT("데이터 소실 안내"), MB_OKCANCEL | MB_ICONQUESTION );
	...
	if( INVALID_HANDLE_VALUE == hFile )
	{
		MessageBox( hWnd, TEXT("파일 저장에 실패했습니다."), TEXT("꺄아아아아아악!"), MB_OK | MB_ICONERROR );
		return E_HANDLE;
	}
	...
	if( bExtChg )	//	拡張子変更した場合
	{
		...
		StringCchPrintf( atBuffer, MAX_STRING, TEXT("%s 확장자로 파일을 저장했습니다."), aatExte[idExten] );
	...
	else
	{
		...
		if( bUnic || bUtf8 )
		{
			NotifyBalloonExist( TEXT("파일을 UTF-8로 내보내기했습니다."), TEXT("일했꼬꼬"), NIIF_INFO );
		}
		else
		{
			if( gbSaveMsgOn ){	NotifyBalloonExist( TEXT("파일을 저장했습니다."), TEXT("일했꼬꼬"), NIIF_INFO );	}
		}
	}
...
HRESULT DocHtmlExport( HWND hWnd )
{
	...
	{
		MessageBox( hWnd, TEXT("내보내기를 하려면 먼저 파일을 저장해야 합니다."), TEXT("저장을 해주세요"), MB_OK | MB_ICONERROR );
		return E_ABORT;
	}
	...
	if( NULL == atExePath[0] )
	{
		MessageBox( hWnd, TEXT("MLT2HTML.exe 파일을 설정해주세요."), TEXT("도구가 없어요"), MB_OK | MB_ICONERROR );
		return E_ABORT;
	}
