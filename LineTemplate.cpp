...
//テンプレデータ読み出し
	...
	//	ウインドウ作成
	ghTmpleWnd = CreateWindowEx( dwExStyle, LINETEMPLATE_CLASS, TEXT("팔레트"),
...
HWND DockingTabCreate( HINSTANCE hInst, HWND hPrWnd, LPRECT pstRect )
{
	...
	stTcItem.pszText = TEXT("팔레트");	TabCtrl_InsertItem( hWorkWnd, 0, &stTcItem );
...
HRESULT DockingTabContextMenu( HWND hWnd, HWND hWndContext, LONG xPos, LONG yPos )
{
	...
	if( gbDockTmplView )	AppendMenu( hPopupMenu, MF_STRING, IDM_LINE_BRUSH_TMPL_VIEW, TEXT("템플릿 비표시") );
...
LRESULT CALLBACK LineTmpleProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	switch( message )
	{
        ...
        //다른 키 누른 상태로 휠 굴리면 프로그램 터져서 주석 처리
